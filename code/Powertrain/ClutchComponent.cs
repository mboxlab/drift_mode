...
public partial class ClutchComponent : PowertrainComponent
{
	...
	/// </summary>
	[Property] public AltCurve EngagementCurve { get; set; }
	...
	{
		base.OnStart();
		SlipTorque = CarController.Powertrain.Engine.EstimatedPeakTorque * 1.5f;
	}
	...
	public override float QueryAngularVelocity( float angularVelocity, float dt )
	{
		...
		{
			EngineComponent engine = CarController.Powertrain.Engine;

			// Engine is at risk of stalling, disconnect the clutch
			if ( CarController.Powertrain.Engine.OutputRPM < engine.IdleRPM )
			{
				ClutchInput = 0f;
			}
			// Override engagement when shifting to smoothly engage and disengage gears
			else if ( CarController.Powertrain.Transmission.IsShifting )
			{
				float shiftProgress = CarController.Powertrain.Transmission.ShiftProgress;
			...
			else
			{
				...
				float throttleInput = CarController.Input.InputSwappedThrottle;
				...
				if ( engine.OutputRPM > engine.IdleRPM * 1.1f && CarController.CurrentSpeed > 3f )
				{
					ClutchInput = 1f;
				}
			}
			if ( CarController.Input.Clutch > 0 )
			{
				ClutchInput = 1 - CarController.Input.Clutch;
			}
		...
		else if ( СontrolType == ClutchControlType.Manual )
		{
			...
			ClutchInput = CarController.Input.Clutch;
		}
	...
	private void ApplyCreepTorque( ref float torque, float creepTorque )
	{
		...
		if ( creepTorque != 0 && CarController.Powertrain.Engine.IsRunning && CarController.CurrentSpeed < CreepSpeedLimit )
		{
