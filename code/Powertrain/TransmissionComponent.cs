...
public class TransmissionComponent : PowertrainComponent
{
	...
	public partial class TransmissionGearingProfile
	{
		...
		/// </summary>
		public List<float> ForwardGears { get; set; } = new() { 3.59f, 2.02f, 1.38f, 1f, 0.87f };

		/// <summary>
		///     List of reverse gear ratios starting from 1st reverse gear.
		/// </summary>
		public List<float> ReverseGears { get; set; } = new() { -4f, };
	}
	...
	[Property] public TransmissionGearingProfile GearingProfile { get; set; }
	...
	/// gear shift function.
	/// </summary>
	public delegate void Shift( CarController vc );
	...
	/// </summary>
	[Property] public TransmissionShiftType TransmissionType { get; set; }
	...
	private float CalculateTotalGearRatio()
	{
		...
		if ( TransmissionType == TransmissionShiftType.CVT )
		{
			...
			float t = Math.Clamp( CarController.Powertrain.Engine.RPMPercent + (1f - CarController.Powertrain.Engine.ThrottlePosition), 0, 1 );
	...
	private float CalculateNoSlipRPM()
	{
		...
		{
			angVelSum += vehicleLocalVelocity / wheelComponent.Wheel.Radius.InchToMeter();
		}

		// Apply total gear ratio to get the no-slip condition RPM
		return UnitConverter.AngularVelocityToRPM( angVelSum / CarController.Powertrain.WheelCount ) * CurrentGearRatio;
	}
	...
	private void ManualShift( CarController car )
	{
		if ( car.Input.ShiftUp )
		{
			ShiftInto( Gear + 1 );
			return;
		}

		if ( car.Input.ShiftDown )
		{
			ShiftInto( Gear - 1 );
			return;
		}
	...
	public void ShiftInto( int targetGear, bool instant = false )
	{
		...
		if ( CarController.Input.Clutch > ClutchInputShiftThreshold )
			return;
	...
	private void CVTShift( CarController car ) => AutomaticShift( car );

	/// <summary>
	///     Handles automatic and automatic sequential shifting.
	/// </summary>
	private void AutomaticShift( CarController car )
	{
		float vehicleSpeed = car.CurrentSpeed;

		float throttleInput = car.Input.InputSwappedThrottle;
		float brakeInput = car.Input.InputSwappedBrakes;
		int currentGear = Gear;
		...
		if ( VariableShiftPoint )
		{
			...
			float revLimiterRPM = car.Powertrain.Engine.RevLimiterRPM;
			...
			_targetDownshiftRPM = Math.Clamp( _targetDownshiftRPM, car.Powertrain.Engine.IdleRPM * 1.1f, _targetUpshiftRPM * 0.7f );
